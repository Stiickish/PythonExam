...
### VÆRSGO DENIS, LAV MIG TIL EN STREAMLIT APP! :-D

x_axis = 'energy'
y_axis = 'danceability'

# Calculate the mean value of danceability for each energy value
df_mean = df.groupby(x_axis)[y_axis].mean().reset_index()

# Create a line chart using Plotly
fig = px.line(df_mean, x=x_axis, y=y_axis, title=f'Line Chart: {y_axis} vs {x_axis}')
fig.update_layout(
    xaxis_title=x_axis,
    yaxis_title=y_axis
)
...
show_energy_button = st.button("Show relations between energy and danceability")
if show_energy_button:
    st.plotly_chart(fig)
